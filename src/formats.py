"""
Ground-truth formats, as they ACTUALLY are.

Every parser here was written against the `ANSWER FORMATS PER TASK` dump from
inspect_data on the real 6,270-row split, not against the paper's figures. Six
of my earlier assumptions were wrong; four would have scored a hard zero. The
corrections, task by task:

TAL              "20.7-55.7 seconds."
                 "66.0-66.0, 118.0-118.0, 124.0-126.0, ... seconds."
                 -> comma-separated on ONE line, single trailing "seconds."
                 -> ZERO-DURATION spans are legal and common (66.0-66.0).
                    The old filter required end > start and dropped them all.

STG              "66.0 seconds: [117.54, 339.56, 626.88, 1005.62]
                  74.0 seconds: [104.48, 313.44, 587.7, 1005.62]"
                 -> a TIME-INDEXED SEQUENCE of boxes, not one box.
                    The old parser took the first and discarded the rest, on
                    780 samples. Coordinates are floats.
...
skill_assessment "Respect for tissue: 4/5, Suture/needle handling: 4/5,
                 ...
                 -> six OSATS dimensions rated 1-5. NOT Novice/Intermediate/
                    Expert, which appear nowhere in the data.

cvs_assessment   "Two structures: 0, Cystic plate: 0, Hepatocystic triangle: 0"
                 -> three criteria scored 0-2. (This one I had right.)

dense_captioning TWO sub-formats:
                   with label    "7.0-22.0 seconds: suturing: <description>"
                   without label "0.0-88.0 seconds: <description>"
                 -> the old regex required both colons and dropped CholecT50,
                    CoPESD and every other unlabelled source.

next_action      free text, 1-5 words: "dissect flap tissue with knife"
region_caption   one sentence
video_summary    one paragraph
"""
